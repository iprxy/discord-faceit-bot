...
export default class Faceit {
  ...
  _calculateAverageStats(segments, mapping) {
    ...
    return stats
      .reduce((acc, curr, i) => Object.keys(curr).reduce((acc2, curr2) => {
        let value;
        if (stats.length === i + 1) {
          if (curr2.includes('average') || curr2.includes('winRate')) {
            value = (acc[curr2].reduce((acc3, curr3) => acc3 + curr3, curr[curr2]) / stats.length)
              .toFixed(2) / 1;
          } else {
            value = (acc[curr2].reduce((acc3, curr3) => acc3 + curr3, curr[curr2]));
          }
        } else {
