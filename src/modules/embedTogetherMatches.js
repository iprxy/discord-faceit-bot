export default function (username, opponentname,togetherMatches) {
  ...
  if (togetherMatches.length > 10) {
    results = `${username} and ${opponentname} played ${togetherMatches.length} matches together, list of last 10 matches:\n` + 
    ...
    results = `${username} and ${opponentname} played ${togetherMatches.length} matches together:\n` + 
      togetherMatches.map(item => `[${item}](https://www.faceit.com/ru/csgo/room/'${item}, '')`).slice(0, 10).join('\n')
  }
